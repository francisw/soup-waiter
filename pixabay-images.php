...
// add settings
include( plugin_dir_path( __FILE__ ) . 'pixabay-settings.php' );

function pixabay_images_enqueue_jquery() {
...
function media_upload_tabs_handler( $tabs ) {
	$tabs['pixabaytab'] = __( 'Pixabay Images', 'pixabay_images' );
...
if ( ! $pixabay_images_settings['button'] | $pixabay_images_settings['button'] == 'true' ) {
	function media_buttons_context_handler( $editor_id = '' ) {
		return '<a href="' . add_query_arg( 'tab', 'pixabaytab', esc_url( get_upload_iframe_src() ) ) . '" id="' . esc_attr( $editor_id ) . '-add_media" class="thickbox button" title="' . esc_attr__( 'Pixabay Images', 'pixabay_images' ) . '"><img style="position:relative;top:-2px" src="' . plugin_dir_url( __FILE__ ) . 'img/favicon.png' . '"> Pixabay</a>';
	}
...
function media_pixabay_images_tab() {
        ...
        <form id="pixabay_images_form" style="margin:0">
            <div style="line-height:1.5;margin:1em 0;max-width:500px;position:relative">
                <input id="q" type="text"  style="width:100%;padding:7px 32px 7px 9px" class="pix-search" value=""
                       autofocus
                       placeholder="<?= htmlspecialchars( __( 'Search for "red roses", "flowers -red", "city OR town", etc.', 'pixabay_images' ) ); ?>">
                <button type="submit"
            ...
            <div style="margin:1em 0;padding-left:2px;line-height:2">
                ...
                <label style="margin-right:20px;white-space:nowrap"><input type="checkbox"
                                                                           id="filter_cliparts"><?= _e( 'Cliparts', 'pixabay_images' ); ?>
                </label>
                <label style="margin-right:15px;white-space:nowrap"><input type="checkbox"
                                                                           id="filter_horizontal"><?= _e( 'Horizontal', 'pixabay_images' ); ?>
                </label>
                <label style="margin-right:25px;white-space:nowrap"><input type="checkbox"
                                                                           id="filter_vertical"><?= _e( 'Vertical', 'pixabay_images' ); ?>
                </label>
                <a id="pixabay_settings_icon" href="options-general.php?page=pixabay_images_settings"
    ...
    <script>
        ...
        var post_id = <?=absint( $_REQUEST['post_id'] ) ?>,
            ...
            per_page = 50, form = jQuery('#pixabay_images_form'), hits, q, image_type, orientation;
...
function media_upload_pixabaytab_handler() {
	wp_iframe( 'media_pixabay_images_tab' );
}
